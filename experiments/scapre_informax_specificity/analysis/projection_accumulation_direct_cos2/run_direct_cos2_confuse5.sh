...
V1_EXPERIMENT="$SCRIPT_DIR/../projection_accumulation"
...
status_run() {
  local run_dir="${1:-}"
  if [[ -z "$run_dir" ]]; then
    [[ -f "$STATE_DIR/latest_run" ]] || { echo "ERROR: no direct-cos2 run recorded" >&2; exit 2; }
  ...
  echo "completed_direct_cos2_images: $scored/15000"
  echo "output: $run_dir"
...
[[ -f "$V1_EXPERIMENT/.server/confuse5/latest_run" ]] || {
  echo "ERROR: completed V1 run pointer is missing" >&2; exit 2;
}
V1_RUN_DIR="$(tr -d '\r\n' < "$V1_EXPERIMENT/.server/confuse5/latest_run")"
[[ -d "$V1_RUN_DIR" && -f "$V1_RUN_DIR/COMPLETED" ]] || {
  echo "ERROR: V1 run is not completed: $V1_RUN_DIR" >&2; exit 2;
}
[[ -f "$V1_RUN_DIR/seeds/20260820/diagnostics/projection_projection_accumulation.pt" ]] || {
  echo "ERROR: V1 seed-20260820 projection diagnostics are missing" >&2; exit 2;
}
cd "$REPO_ROOT"
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || {
  echo "ERROR: formal launch requires a clean working tree" >&2; git status --short >&2; exit 2;
}
"$PYTHON_BIN" -c 'import torch; assert torch.cuda.is_available(), "CUDA unavailable"'
"$PYTHON_BIN" -m py_compile \
  "$V1_EXPERIMENT/projection_runner.py" \
  "$V1_EXPERIMENT/evaluate_projection_runner.py" \
  "$V1_EXPERIMENT/aggregate_results.py" \
  "$V1_EXPERIMENT/worker.py" \
  "$SCRIPT_DIR/analyze_v1_diagnostics.py"
OFFICIAL_REFERENCE="$("$PARENT_EXPERIMENT/analysis/alpha_channel_controls/resolve_official_reference.sh")"
[[ -d "$OFFICIAL_REFERENCE" ]] || { echo "ERROR: official reference resolution failed" >&2; exit 2; }
if [[ "$ACTION" == "--preflight" ]]; then
  echo "Direct-cos2 server preflight passed. No experiment was launched."
  echo "Python: $PYTHON_BIN"
  echo "V1 diagnostics: $V1_RUN_DIR"
...
if [[ -f "$STATE_DIR/active_run" ]]; then
  ...
  if [[ -f "$ACTIVE/pid" && ! -f "$ACTIVE/COMPLETED" && ! -f "$ACTIVE/FAILED" ]]; then
    ...
    if [[ "$ACTIVE_PID" =~ ^[0-9]+$ ]] && kill -0 "$ACTIVE_PID" 2>/dev/null; then
      echo "ERROR: direct-cos2 run already active: $ACTIVE (PID $ACTIVE_PID)" >&2; exit 2
    fi
  fi
fi
if [[ "$ACTION" == "--resume" ]]; then
  ...
  [[ -d "$RUN_DIR" && ! -f "$RUN_DIR/COMPLETED" ]] || { echo "ERROR: run cannot be resumed: $RUN_DIR" >&2; exit 2; }
else
...
[[ "$RUN_DIR" == "$RUNS_DIR"/formal_* ]] || { echo "ERROR: run path escaped runs root" >&2; exit 2; }
mkdir -p "$RUN_DIR/logs"
...
nohup "$SCRIPT_DIR/server_worker.sh" "$RUN_DIR" "$ASSETS" "$OFFICIAL_REFERENCE" "$V1_RUN_DIR" \
...
echo "Started V1 diagnostics -> qualification -> direct-cos2 Confuse5 formal. Safe to disconnect."
echo "PID: $PID"
echo "Output: $RUN_DIR"
echo "Log: $RUN_DIR/logs/server.log"
echo "Status: $SCRIPT_DIR/run_direct_cos2_confuse5.sh --status"
