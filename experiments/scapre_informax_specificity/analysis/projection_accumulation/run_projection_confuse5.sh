...
status_run() {
  local run_dir="${1:-}"
  if [[ -z "$run_dir" ]]; then
    [[ -f "$STATE_DIR/latest_run" ]] || { echo "ERROR: no Confuse5 run recorded" >&2; exit 2; }
  ...
  done < <(find "$run_dir/seeds" -path '*/evaluation/projection_accumulation/scores.csv' -type f 2>/dev/null | sort)
  echo "status: $state"
  echo "stage: $stage"
  echo "pid: $pid"
  echo "exit_code: $exit_code"
  echo "completed_seeds: $seeds"
  echo "completed_projection_images: $scored/15000"
  echo "output: $run_dir"
...
if [[ "$ACTION" != "launch" && "$ACTION" != "--resume" ]]; then
  echo "usage: $0 [launch|--resume|--status|--package] [run-id-or-run-dir]" >&2
  exit 2
fi
...
if [[ -f "$STATE_DIR/active_run" ]]; then
  ...
  if [[ -f "$ACTIVE/pid" && ! -f "$ACTIVE/COMPLETED" && ! -f "$ACTIVE/FAILED" ]]; then
    ...
    if [[ "$ACTIVE_PID" =~ ^[0-9]+$ ]] && kill -0 "$ACTIVE_PID" 2>/dev/null; then
      echo "ERROR: Confuse5 run already active: $ACTIVE (PID $ACTIVE_PID)" >&2; exit 2
    fi
  fi
fi
if [[ "$ACTION" == "--resume" ]]; then
  if [[ -n "${2:-}" ]]; then
    ...
    [[ -f "$STATE_DIR/latest_run" ]] || { echo "ERROR: no Confuse5 run available to resume" >&2; exit 2; }
    RUN_DIR="$(tr -d '\r\n' < "$STATE_DIR/latest_run")"
  fi
...
[[ "$RUN_DIR" == "$RUNS_DIR"/formal_* ]] || { echo "ERROR: run path escaped Confuse5 runs root" >&2; exit 2; }
mkdir -p "$RUN_DIR/logs"
...
echo "Started qualification -> Confuse5 formal. Safe to disconnect."
echo "PID: $PID"
echo "Output: $RUN_DIR"
echo "Log: $RUN_DIR/logs/server.log"
echo "Status: $SCRIPT_DIR/run_projection_confuse5.sh --status"
