...
def validate_configuration(config: dict[str, Any], base: dict[str, Any]) -> None:
    ...
    if formula["temperature"] != 0.7 or formula["power"] != 8.0 or formula["eps"] != 1e-8:
    ...
    if paths["completed"].exists():
        ...
        if not paths["checkpoint"].is_file():
            ...
            projection_evaluation = (
                seed_dir / "evaluation" / "projection_accumulation" / "COMPLETED"
            )
...
def validate_edit_isolation(seed_dir: Path, commands: dict[str, list[str]]) -> dict[str, Any]:
    if normalized_command(commands["official"]) != normalized_command(commands["projection_accumulation"]):
    ...
    for key in (
        ...
        if audits["official"][key] != audits["projection_accumulation"][key]:
    ...
    right_records = projection["projection_accumulation"]["accumulation_records"]
    ...
    compare_fields = (
        ...
        "official_row_w_c", "projection_score", "projection_alpha",
    ...
    right_matrices = projection["projection_accumulation"]["matrix_records"]
    ...
    assert_equal(informax["official"], informax["projection_accumulation"], "production_informax")
    completion = {
        variant: json.loads((seed_dir / "stages" / f"edit_{variant}.completed.json").read_text())
        for variant in VARIANTS
    }
    if completion["official"]["checkpoint_sha256"] == completion["projection_accumulation"]["checkpoint_sha256"]:
...
def qualification_report(
    ...
    production_hash_start: str,
) -> dict[str, Any]:
    output.mkdir(parents=True, exist_ok=True)
    wrapper = torch.load(seed_dir / "diagnostics" / "projection_projection_accumulation.pt", map_location="cpu")
    ...
    projection_alpha = torch.cat([row["projection_alpha"].flatten() for row in records])
    ...
    with correlations.open("w", newline="") as handle:
        fields = ["projection", "layer_index", "target_index", "target_concept", "pearson", "spearman"]
    ...
    report = {
        ...
        "selection_criteria": {
            ...
            "all_projection_alpha_finite": bool(torch.isfinite(projection_alpha).all().item()),
            "projection_alpha_non_constant": bool(projection_alpha.max().item() != projection_alpha.min().item()),
        ...
        "projection_alpha_distribution": distribution(projection_alpha),
...
def evaluate(seed_dir: Path, base_config: Path, assets: Path, protocol: Path) -> None:
    output = seed_dir / "evaluation" / "projection_accumulation"
    if (output / "COMPLETED").exists():
        rows = read_csv(output / "scores.csv")
        if len(rows) != 3000 or any(row["variant"] != "projection_accumulation" for row in rows):
    ...
    run([
        ...
        "--checkpoint", str(seed_dir / "checkpoints" / "projection_accumulation.pt"),
...
def main() -> None:
    ...
    if manifest_path.exists():
        ...
        for key in ("git_commit", "git_branch", "source_sha256", "assets", "assets_sha256"):
            if previous.get(key) != manifest[key]:
                raise RuntimeError(f"resume provenance changed: {key}")
        manifest = previous
    else:
        write_json(manifest_path, manifest)
    shutil.copy2(CONFIG, run_dir / "reproducibility" / "actual_config.json")
    ...
    for seed in SEEDS:
        ...
        evaluate(seed_dir, run_dir / "reproducibility" / "base_config.json", assets_path, protocol)
        projection_manifest = (
            seed_dir / "evaluation" / "projection_accumulation" / "evaluation_manifest.json"
        )
    ...
    integrity = {
        "status": "passed", "edit_seeds": SEEDS,
