...
RUN_DIR="${1:-}"
if [[ -z "$RUN_DIR" ]]; then
  [[ -f "$SCRIPT_DIR/.server/confuse5/latest_run" ]] || {
    echo "ERROR: no recorded run" >&2
    exit 2
  }
...
[[ -n "$RUNS_ROOT" && -n "$RUN_DIR" && "$RUN_DIR" == "$RUNS_ROOT"/* ]] || {
  echo "ERROR: package target must be one explicit budget-matched-cos2 run" >&2
  exit 2
}
...
[[ -f "$RUN_DIR/reproducibility/integrity_report.json" ]] || {
  echo "ERROR: integrity report missing" >&2
  exit 2
}
INTEGRITY_STATUS="$(sed -n 's/^  "status": "\([^"]*\)",\{0,1\}$/\1/p' "$RUN_DIR/reproducibility/integrity_report.json" | head -n 1)"
[[ "$INTEGRITY_STATUS" == "passed" ]] || {
  echo "ERROR: integrity report did not pass" >&2
  exit 2
}
...
FILES=(
  ...
  qualification/per_layer_concept_budget_matching.csv
  ...
  results/historical_variant_comparison.csv results/validation_report.md
...
while IFS= read -r source; do
  FILES+=("reproducibility/provenance/$source")
done < <(sed -n '/"source_sha256": {/,/^  }/s/^    "\([^"]*\)".*/\1/p' "$RUN_DIR/reproducibility/run_manifest.json")
for relative in "${FILES[@]}"; do
  [[ -f "$RUN_DIR/$relative" ]] || {
    echo "ERROR: required package file missing: $relative" >&2
    exit 2
  }
done
...
mkdir -p "$ARCHIVE_DIR"
ARCHIVE="$ARCHIVE_DIR/scapre_projection_budget_matched_cos2_$(basename "$RUN_DIR")_$(date -u +'%Y%m%dT%H%M%SZ').tar.gz"
