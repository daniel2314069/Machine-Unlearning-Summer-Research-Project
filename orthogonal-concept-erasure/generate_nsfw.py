...
def generate_images(model_id, oce_model_path, prompts_path, save_path, exp_name='test', device='cuda:0', torch_dtype=torch.bfloat16, guidance_scale = 7.5, num_inference_steps=50, num_images_per_prompt=10, from_case=0, till_case=1000000):
    ...
    for i, row in df.iterrows():
        try:
            ...
            for num, im in enumerate(pil_images):
                im.save(f"{folder_path}/{case_number}.png")
        except:
            prompt=str(row.adv_prompt)
            pil_images = pipe(prompt=prompt,
                            num_inference_steps=num_inference_steps,
                            guidance_scale=guidance_scale,
                            num_images_per_prompt=num_images_per_prompt,
...
if __name__=='__main__':
    ...
    generate_images(model_id=model_id, oce_model_path=oce_model_path, prompts_path=prompts_path, save_path=save_path, exp_name=exp_name, device=device, torch_dtype=torch.bfloat16, guidance_scale = guidance_scale, num_inference_steps=num_inference_steps, num_images_per_prompt=num_images_per_prompt, from_case=from_case, till_case=till_case)