...
if [[ -s "$PID_FILE" && ! -s "$EXIT_FILE" ]]; then
    ...
    if [[ "$EXISTING_PID" =~ ^[0-9]+$ ]] && kill -0 "$EXISTING_PID" 2>/dev/null; then
        echo "A follow-up OCE job is already running with PID $EXISTING_PID." >&2
...
LOG_FILE="$LOG_DIR/sequential_oce_followup_${TIMESTAMP}.log"
: > "$EXIT_FILE"
...
echo "Sequential OCE follow-up started in the background."
echo "PID: $JOB_PID"
echo "Log: $LOG_FILE"
